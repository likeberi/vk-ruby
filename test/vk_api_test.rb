...
describe VkApi do
  ...
  describe VkApi::FrequencyControl do
    ...
    describe '@@counter' do
      it "should exist" do
        assert_equal VkApi::FrequencyControl.class_variables.include?(:@@counter), true
      end

      it 'should be empty after start' do
        assert_equal VkApi::FrequencyControl.class_variable_get(:@@counter), {}
      end
    end
    ...
    describe 'request_can_be_executed_now?' do
      it 'should return true if counter is empty' do
        ...
        VkApi::FrequencyControl.class_variable_set(:@@counter, {})
      ...
      it 'should return true if no requests for current second' do
        session = VkApi::FrequencyControl
        VkApi::FrequencyControl.class_variable_set(:@@counter, {'token' => [Time.now.to_f - 2]})
      ...
      it 'should return true if less than 3 requests for current second executed' do
        session = VkApi::FrequencyControl
        time = Time.now.to_f
        VkApi::FrequencyControl.class_variable_set(:@@counter, {'token' => [time, time]})
        assert_equal session.request_can_be_executed_now?(time, 'token'), true
      end
    ...
    describe 'update_counter' do
      it 'should set current time if counter is empty' do
        ...
        assert_equal  VkApi::FrequencyControl.class_variable_get(:@@counter), { 'token' => [time] }

        VkApi::FrequencyControl.class_variable_set(:@@counter, nil)
        session.update_counter(time, 'token')
        assert_equal  VkApi::FrequencyControl.class_variable_get(:@@counter), { 'token' =>[time] }
      end
      ...
      it 'should set first time to token if no token in current time' do
        time = Time.now.to_i
        VkApi::FrequencyControl.class_variable_set(:@@counter, { 'token0' => [time]})
        session = VkApi::FrequencyControl
        session.update_counter(time, 'token1')

        assert_equal  VkApi::FrequencyControl.class_variable_get(:@@counter), { 'token0' => [time], 'token1' => [time] }
      end

      it 'should add time to token if token exists' do
        time = Time.now.to_f
        VkApi::FrequencyControl.class_variable_set(:@@counter, { 'token' => [time - 1, time]})
        session = VkApi::FrequencyControl
        session.update_counter(time, 'token')

        assert_equal  VkApi::FrequencyControl.class_variable_get(:@@counter), { 'token' => [time - 1, time, time] }
      end
    end
  end
end
